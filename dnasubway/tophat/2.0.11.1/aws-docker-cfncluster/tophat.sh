...
# Ported to use LS4 Docker image. This is really ugly.
# All binary deps should be in a new Docker image
# and the heavy bash logic should be in a workflow script
...
if [ "$PE" = "1" ]; then
    ...
    $DOCKER_APP_RUN perl scripts/resynch_paired_reads.pl $QUERY1_F $QUERY2_F
...
QUAL=$($DOCKER_APP_RUN perl scripts/check_qual_score.pl $QUERY1_F)
if [[ -n $QUAL ]]; then
...
$DOCKER_APP_RUN tophat $ARGS 2> tophat.stderr
exit_code=$?
...
$DOCKER_APP_RUN samtools sort  "$output_dir/accepted_hits.bam" "$output_dir/accepted_hits_sorted"
$DOCKER_APP_RUN samtools index "$output_dir/accepted_hits_sorted.bam"
$DOCKER_APP_RUN samtools flagstat "$output_dir/accepted_hits_sorted.bam" > "$output_dir/flagstat_out.txt"

realName=""
if [ "$PE" = "1" ]; then
    realName=$($DOCKER_APP_RUN perl scripts/build_name.pl $QUERY1_F $QUERY2_F)
else
    name=${QUERY1_F/\.*/}
